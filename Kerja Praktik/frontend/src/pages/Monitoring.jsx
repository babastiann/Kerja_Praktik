...
import { Activity, AlertTriangle, CheckCircle, Cpu, Clock, Loader2 } from "lucide-react";
import { StatCard, SectionHeader } from "../components/ui";
import { useApi } from "../hooks/useApi";
import { apiMonitoring } from "../utils/api";
...
export default function Monitoring() {
  const { data, loading, error } = useApi(apiMonitoring);
  ...
  if (error) return (
    <div className="card p-6 text-center text-accent-rose">
      <p className="font-display font-semibold">Gagal memuat data monitoring</p>
      <p className="text-xs text-surface-500 mt-1">{error}</p>
    </div>
  );

  const { harian = [], model_health = [] } = data || {};
  const latest = harian[harian.length - 1] || {};
  const prev   = harian[harian.length - 2] || {};
  const drift  = latest.r2 && prev.r2 ? (latest.r2 - prev.r2).toFixed(3) : "0.000";
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="MAE Hari Ini"    value={latest.mae ? `${latest.mae} Jt` : "—"} sub="rata-rata absolut" icon={Cpu}  color="orange" />
        <StatCard label="Request Hari ini" value={latest.total_req || "—"}    sub="prediksi diproses" icon={Clock}        color="purple" />
        <StatCard label="Status Model"    value="Normal"                      sub="tidak ada drift"   icon={CheckCircle} color="teal"   />
      </div>

      {/* R² monitoring */}
      <div className="card p-5">
        <SectionHeader title="R² Score — Harian" sub="Deteksi model drift secara real-time">
          <div className={`badge ${+drift >= 0 ? "badge-green" : "badge-rose"}`}>
            {+drift >= 0 ? "↑" : "↓"} {Math.abs(drift)} drift
          </div>
        </SectionHeader>
        <ResponsiveContainer width="100%" height={200}>
          <AreaChart data={harian} margin={{ top: 5, right: 0, left: -25, bottom: 0 }}>
            <defs>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card p-5">
          ...
          <ResponsiveContainer width="100%" height={170}>
            <LineChart data={harian} margin={{ top: 5, right: 0, left: -25, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
              <XAxis dataKey="tanggal" tick={{ fill: "#64748b", fontSize: 9 }} axisLine={false} tickLine={false} />
        ...
        <div className="card p-5">
          ...
          <ResponsiveContainer width="100%" height={170}>
            <AreaChart data={harian} margin={{ top: 5, right: 0, left: -25, bottom: 0 }}>
              ...
              <YAxis tick={{ fill: "#64748b", fontSize: 9 }} axisLine={false} tickLine={false} />
              <Tooltip content={<CustomTooltip />} />
              <Area type="monotone" dataKey="total_req" name="Request" stroke="#a855f7" fill="url(#gradReq)" strokeWidth={2} dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
              ...
              <tbody>
                {model_health.map(m => (
                  <tr key={m.model_name} className="table-row">
                    <td className="table-td font-display font-semibold text-surface-100">{m.model_name}</td>
                    <td className="table-td font-mono text-brand-400">{m.r2_score}</td>
                    ...
                    <td className="table-td">
                      <span className={m.r2_score >= 0.85 ? "badge-green" : m.r2_score >= 0.75 ? "badge-orange" : "badge-rose"}>
                        {m.r2_score >= 0.85 ? "Excellent" : m.r2_score >= 0.75 ? "Good" : "Fair"}
                      </span>
                    </td>
                  </tr>
                ))}
